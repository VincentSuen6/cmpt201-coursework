...
// divide words into chunks, one per thread
size_t chunk = num_words / THREAD_COUNT;

for (int i = 0; i < THREAD_COUNT; i++) {
    threads_args[i] = pack_args(&map, words + i*chunk, chunk, &count_mutex);
...
for (size_t i = 0; i < num_words; i++) {
    pthread_mutex_lock(lock);      // ← add this
    ...
    pthread_mutex_unlock(lock);    // ← add this
}
